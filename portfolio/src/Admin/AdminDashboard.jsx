...
// ─── Palette ──────────────────────────────────────────────────────────────────
const CHART_COLORS = ["#00f0ff", "#a855f7", "#10b981", "#f59e0b", "#ef4444", "#3b82f6"];
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      background: "rgba(10,12,20,0.95)", border: "1px solid rgba(0,240,255,0.25)",
      borderRadius: 10, padding: "0.7rem 1rem", fontSize: "0.85rem", color: "#e2e8f0"
    }}>
      <p style={{ margin: 0, fontWeight: 700, color: "#00f0ff", marginBottom: 4 }}>{label}</p>
      {payload.map((p, i) => (
...
const StatCard = ({ icon, label, value, accent, sub }) => (
  <div style={{
    ...
    border: `1px solid ${accent}33`, borderRadius: 16, padding: "1.5rem",
    boxShadow: `0 0 20px ${accent}18`, display: "flex", alignItems: "center", gap: "1.2rem",
  }}>
    <div style={{
      width: 52, height: 52, borderRadius: 12, background: `${accent}18`,
      display: "flex", alignItems: "center", justifyContent: "center",
      fontSize: "1.5rem", flexShrink: 0
    }}>{icon}</div>
    <div>
      <div style={{ fontSize: "0.8rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>{label}</div>
...
const ChartCard = ({ title, children, style }) => (
  <div style={{
    ...
    boxShadow: "0 8px 30px rgba(0,0,0,0.3)", ...style
  }}>
    <h4 style={{ margin: "0 0 1.2rem", fontSize: "1rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em" }}>{title}</h4>
    {children}
  </div>
);

// ─── Browser / OS / Device icons ─────────────────────────────────────────────
const BROWSER_ICON = { Chrome: "🌐", Firefox: "🦊", Safari: "🧭", Edge: "🌀", Opera: "🔴", Unknown: "❓" };
const OS_ICON = { Windows: "🪟", macOS: "🍎", Linux: "🐧", Android: "🤖", iOS: "📱", Unknown: "💻" };
...
const VisitorCard = ({ session }) => {
  ...
  return (
    <div style={{
      ...
      background: "rgba(15,18,28,0.7)", backdropFilter: "blur(10px)",
      boxShadow: `0 0 18px ${accent}15`, transition: "box-shadow 0.3s"
    }}>
      {/* ── Header ── */}
      <div
      ...
      >
        ...
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: "1.2rem"
        }}>{DEVICE_ICON[meta.device] || "💻"}</div>

        {/* Info */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
            <span style={{ fontWeight: 700, color: accent, fontFamily: "monospace", fontSize: "0.9rem" }}>
              {maskIP(session.ip)}
            </span>
          ...
          <div style={{ fontSize: "0.78rem", color: "#64748b", marginTop: 3 }}>
            ...
            First: {fmtTime(firstSeen)} &nbsp;·&nbsp; Last: {fmtTime(lastSeen)}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", justifyContent: "flex-end", maxWidth: 200 }}>
          {pageList.slice(0, 3).map((p, i) => (
            <span key={i} style={{
              fontSize: "0.7rem", padding: "2px 7px", borderRadius: 6,
              background: "rgba(255,255,255,0.06)", color: "#94a3b8", whiteSpace: "nowrap"
            }}>{p === "/" ? "Home" : p.replace(/^\//, "")}</span>
          ))}
          {pageList.length > 3 && <span style={{ fontSize: "0.7rem", color: "#64748b" }}>+{pageList.length - 3}</span>}
        </div>

        <span style={{ color: "#475569", fontSize: "0.9rem", marginLeft: "0.5rem", flexShrink: 0 }}>
          {expanded ? "▲" : "▼"}
        </span>
      </div>
      ...
      {/* ── Expanded Detail ── */}
      {expanded && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "1rem 1.4rem" }}>
          {/* Metadata grid */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "0.8rem", marginBottom: "1rem" }}>
            {[
              ...
              <div key={label} style={{
                ...
                <div style={{ fontSize: "0.85rem", color: "#e2e8f0", wordBreak: "break-all" }}>{value}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginBottom: "0.8rem" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem", alignItems: "center" }}>
              {session.events.slice().reverse().map((ev, i) => (
                <>
                  <span key={`p${i}`} style={{
                    fontSize: "0.78rem", padding: "3px 10px", borderRadius: 6,
                    background: `${accent}18`, color: accent, border: `1px solid ${accent}33`
                  }}>{ev.path === "/" ? "Home" : ev.path.replace(/^\//, "")}</span>
                  {i < session.events.length - 1 && (
                    <span key={`a${i}`} style={{ color: "#475569", fontSize: "0.75rem" }}>→</span>
                  )}
                </>
              ))}
            </div>
          ...
          <button
            ...
            style={{
              ...
              color: "#64748b", fontSize: "0.75rem", padding: "4px 10px", borderRadius: 6,
              cursor: "pointer", marginBottom: showRaw ? "0.6rem" : 0
            }}
          >{showRaw ? "Hide" : "Show"} raw metadata</button>

          {showRaw && (
            <pre style={{
              background: "rgba(0,0,0,0.4)", borderRadius: 8, padding: "0.8rem",
              fontSize: "0.72rem", color: "#94a3b8", overflowX: "auto", margin: 0,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", paddingBottom: "3rem" }}>
      <p style={{ fontSize: "0.85rem", color: "#64748b", margin: "0 0 0.5rem" }}>
        {sessions.length} unique visitor{sessions.length !== 1 ? "s" : ""} tracked
        &nbsp;·&nbsp; IPs are partially masked for display
      </p>
      {sessions.map((s, i) => <VisitorCard key={s.ip + i} session={s} />)}
    </div>
  );
};
...
const AnalyticsDashboard = ({ items, loading }) => {
  ...
  const stats = useMemo(() => {
    ...
    const todayCount = items.filter(e => e.timestamp?.slice(0, 10) === today).length;

    // Unique visitors (unique IPs)
    const uniqueIPs = new Set();
    items.forEach(e => { try { const m = JSON.parse(e.metadata_json || "{}"); if (m.ip) uniqueIPs.add(m.ip); } catch {} });

    // Unique paths
    const pathCounts = {};
    items.forEach(e => { pathCounts[e.path] = (pathCounts[e.path] || 0) + 1; });
    ...
    // Event types pie
    const typeCounts = {};
    items.forEach(e => { typeCounts[e.event_type] = (typeCounts[e.event_type] || 0) + 1; });
    const pieData = Object.entries(typeCounts).map(([name, value]) => ({ name, value }));

    // Recent events
    const recent = [...items].slice(0, 15);

    return { total, pageViews, todayCount, topPage, timelineData, topPages, pieData, recent, uniqueVisitors: uniqueIPs.size };
  }, [items]);

  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, color: "#94a3b8", padding: "2rem" }}>
      <div style={{ width: 18, height: 18, borderRadius: "50%", border: "2px solid #00f0ff", borderTopColor: "transparent", animation: "spin 0.8s linear infinite" }} />
  ...
  if (!items.length) return (
    <div style={{ textAlign: "center", color: "#64748b", padding: "4rem 2rem" }}>
      ...
      <p style={{ fontSize: "0.9rem" }}>Events will appear here as visitors browse your portfolio.</p>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", paddingBottom: "3rem" }}>
      {/* ── Sub-tab switcher ── */}
      <div style={{ display: "flex", gap: "0.5rem", background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "4px", width: "fit-content" }}>
        {[["overview", "📊 Overview"], ["visitors", "👤 Visitors"]].map(([key, label]) => (
          <button key={key} onClick={() => setView(key)} style={{
            padding: "0.45rem 1.1rem", borderRadius: 8, border: "none", cursor: "pointer", fontSize: "0.9rem",
            fontWeight: 600, transition: "all 0.2s",
            background: view === key ? "rgba(0,240,255,0.12)" : "transparent",
            color: view === key ? "#00f0ff" : "#64748b",
            boxShadow: view === key ? "0 0 12px rgba(0,240,255,0.15)" : "none"
          }}>{label}</button>
        ))}
      </div>

      {view === "visitors" ? <VisitorsView items={items} /> : (
        <>
          {/* KPI CARDS */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem" }}>
            <StatCard icon="👁️" label="Total Events" value={stats.total.toLocaleString()} accent="#00f0ff" sub="All tracked interactions" />
            <StatCard icon="👤" label="Unique Visitors" value={stats.uniqueVisitors.toLocaleString()} accent="#a855f7" sub="By IP address" />
            <StatCard icon="📅" label="Today" value={stats.todayCount.toLocaleString()} accent="#10b981" sub={new Date().toLocaleDateString("en-US", { weekday: "long" })} />
            <StatCard
              icon="🏆" label="Top Page"
              value={stats.topPage ? (stats.topPage[0] === "/" ? "Home" : stats.topPage[0].replace(/^\//, "")) : "—"}
              accent="#f59e0b"
          ...
          <ChartCard title="Visits — Last 7 Days">
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={stats.timelineData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="visits" stroke="#00f0ff" strokeWidth={2} fill="url(#areaGrad)" dot={{ fill: "#00f0ff", r: 3 }} activeDot={{ r: 5 }} />
              </AreaChart>
            </ResponsiveContainer>
          </ChartCard>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
            <ChartCard title="Top Pages">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={stats.topPages} layout="vertical" margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" horizontal={false} />
                  <XAxis type="number" tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
                  <YAxis type="category" dataKey="path" tick={{ fill: "#94a3b8", fontSize: 11 }} axisLine={false} tickLine={false} width={70} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="count" name="Hits" radius={[0, 6, 6, 0]}>
                    {stats.topPages.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </ChartCard>
              ...
              {stats.recent.map((ev, i) => {
                ...
                return (
                  <div key={ev.id ?? i} style={{
                    ...
                    <span style={{ fontSize: "1.1rem", flexShrink: 0 }}>{isView ? "📄" : "🖱️"}</span>
                    <span style={{
                      display: "inline-block", padding: "2px 8px", borderRadius: 6, fontSize: "0.72rem",
                      fontWeight: 700, background: isView ? "rgba(0,240,255,0.12)" : "rgba(168,85,247,0.12)",
                      color: isView ? "#00f0ff" : "#a855f7", flexShrink: 0, whiteSpace: "nowrap"
                    }}>{ev.event_type}</span>
                    <span style={{ flex: 1, color: "#e2e8f0", fontSize: "0.9rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.path}</span>
                    {meta.ip && (
                      <span style={{ fontFamily: "monospace", fontSize: "0.75rem", color: ipColor(meta.ip), flexShrink: 0 }}>
                        {maskIP(meta.ip)}
                      </span>
                    )}
                    <span style={{ color: "#475569", fontSize: "0.78rem", flexShrink: 0, whiteSpace: "nowrap" }}>
